...
import "./NestOpenMiningStorage.sol";

/// @dev This contract implemented the mining logic of nest
contract NestBatchMiningNew is NestOpenMiningStorage {
    ...
    /// @param sheets Target sheets which to search in
    ...
    function _findPrice(
        ...
        height -= priceEffectSpan;
        {
            ...
            // Compatible with history data of peth&nest
            if (index == 0 && height < sheets[0].height && channelId == 0 && pairIndex < 2) {
                // index == 0 && height < sheets[0].height means not found
                // channelId == 0 && pairIndex < 2 means peth or nest

                // This contract is upgraded from the NestOpenMining. In its historical sheets, 
                // each channel has only one token, channelId 0 is peth|pusd, channelId 1 is nest|pusd
                // In this new contract, there many tokens in each channel, when deployed, 
                // we will open a new channel 0, it is support peth&nest&pbtc|pusd, the pairIndex are 0, 1, 2.
                // When target token is peth or nest, and not found in new sheets, this will find in historical sheets
                // from NestOpenMining, and the pirIndex 0 and 1 are convert to channelId.
                return _findPrice(_oldChannels[pairIndex].sheets, height + priceEffectSpan, 1, 2);
            }
        }
