    ...
    function _findPrice(
        ...
        height -= priceEffectSpan;
        {
            ...
            if (index == 0 && height < sheets[0].height && channelId == 0 && pairIndex < 2) {
                // index == 0 && height < sheets[0].height 表示没有找到
                // channelId == 0 && pairIndex < 2表示是peth或nest报价通道
                // 此合约是从之前的单币种报价升级而来，在其历史报价数据中，每个轨道只有一个币种的报价
                // 其中channelId为0的是peth/pusd的报价，channelId为1的是nest/pusd的报价
                // 在新的合约中，一个轨道将支持多币种报价，合约部署时后，会开通0号报价通道，支持peth&nest&pbtc|pusd报价
                // peth，nest，pbtc的pairIndex分别是0，1，2
                // 对于peth和nest的报价对，如果在_channels中没有找到价格，则会在老的报价数据中继续查找，而0，1两个pairIndex
                // 分别转化为老的channelId，在_oldChannels中查找
