...
interface INestBatchPrice {
    ...
    function triggeredPrice(uint channelId, uint pairIndex, address payback) external payable returns (uint blockNumber, uint price);
    ...
    function lastPriceList(uint channelId, uint pairIndex, uint count, address payback) external payable returns (uint[] memory);
    ...
    /// assumes that the volatility cannot exceed 1. Correspondingly, when the return value is equal to 
    /// 999999999999996447, it means that the volatility has exceeded the range that can be expressed
    function lastPriceListAndTriggeredPriceInfo(uint channelId, uint pairIndex, uint count, address payback) external payable 
    returns (
        uint[] memory prices,
        uint triggeredPriceBlockNumber,
        uint triggeredPriceValue,
