...
describe('NestOpenMining', function() {
    it('First', async function() {
        ...
        if (true) {
            ...
            await nestBatchMining.connect(addr1).takeToken0(0, 0, 0, 1, toBigInt(4400, 6), {
                value: toBigInt(2 - 1)
            });
            ...
            if (false) {
                ...
                expect(toDecimal(await nestBatchMining.balanceOf(usdt.address, owner.address), 6)).eq(toDecimal(toBigInt(4300 * 2, 6), 6));
                ...
                console.log('2. 吃单链');
                await nestBatchMining.takeToken0(0, 0, 1, 2, toBigInt(4150, 6), { value: toBigInt(2) });
                status = await showStatus();
                ...
                expect(toDecimal(await nestBatchMining.balanceOf(usdt.address, owner.address), 6)).eq(toDecimal(toBigInt(4300 * 2 + 4150 * 4, 6), 6));
                expect(toDecimal(await nestBatchMining.balanceOf(nest.address, owner.address))).eq(toDecimal(toBigInt(100000 + 400000 + 10)));

                expect(toDecimal(await nestBatchMining.balanceOf(usdt.address, addr1.address), 6)).eq(toDecimal(toBigInt(4400 * 4, 6), 6));
                expect(toDecimal(await nestBatchMining.balanceOf(nest.address, addr1.address))).eq(toDecimal(toBigInt(200000)));
