...
describe('NestOpenMining', function() {
    it('First', async function() {
        ...
        if (true) {
            console.log('1. post');
            let receipt = await nestBatchMining.post(0, 1, [toBigInt(0.459)], {
            ...
            await nestBatchMining.connect(addr1).takeToken0(0, 0, 0, 1, toBigInt(0.5), {
            ...
            if (false) {
                ...
                console.log('2. 吃单链');
                await nestBatchMining.takeToken0(0, 0, 1, 2, toBigInt(0.4), { value: toBigInt(0.4 * 4 + 0.5 * 2) });
                status = await showStatus();
                ...
                expect(toDecimal(await nestBatchMining.balanceOf(usdt.address, owner.address), 6)).eq(toDecimal(toBigInt(2000 * 4, 6), 6));
