...
describe('NestOpenMining', function() {
    it('First', async function() {
        ...
        if (true) {
            console.log('1. post');
            let receipt = await nestOpenMining.post(0, 1, toBigInt(0.459), {
                value: toBigInt(POSTFEE) + 1000000000n * GASLIMIT + toBigInt(0.459)
            });
            ...
            expect(status.mining.nest).to.eq(toDecimal(toBigInt(5000000000 + 1000 + 100000)));
            expect(status.mining.eth).to.eq(toDecimal(toBigInt(POSTFEE) + 1000000000n * GASLIMIT + toBigInt(0.459)));
            ...
            await nestOpenMining.connect(addr1).takeToken0(0, 0, 1, toBigInt(0.5), {
                value: toBigInt(0.5 * 2 + 0.459)
            });
            ...
            expect(status.mining.usdt).to.eq(toDecimal(toBigInt(2000 + 2000, 6), 6));
            expect(status.mining.nest).to.eq(toDecimal(toBigInt(5000000000 + 1000 + 100000 + 200000)));
            expect(status.mining.eth).to.eq(toDecimal(toBigInt(POSTFEE) + 1000000000n * GASLIMIT + toBigInt(1.918)));
            ...
            if (false) {
                ...
                console.log('2. 吃单链');
                await nestOpenMining.takeToken0(0, 1, 2, toBigInt(0.4), { value: toBigInt(0.4 * 4 + 0.5 * 2) });
                status = await showStatus();
                ...
                expect(status.addr1.usdt).to.eq(toDecimal(toBigInt(10000000 - 2000 * 2 + 2000, 6), 6));
                expect(status.addr1.nest).to.eq(toDecimal(toBigInt(1000000000 - 200000)));
                expect(status.mining.usdt).to.eq(toDecimal(toBigInt(2000 + 2000 + 2000 * 4 - 2000 * 2, 6), 6));
                expect(status.mining.nest).to.eq(toDecimal(toBigInt(5000000000 + 1000 + 100000 + 200000 + 400000)));
                expect(status.mining.eth).to.eq(toDecimal(toBigInt(POSTFEE) + 1000000000n * GASLIMIT + toBigInt(1.918 + 0.4 * 4 + 0.5 * 2)));
                ...
                expect(toDecimal(await nestOpenMining.balanceOf(usdt.address, owner.address), 6)).eq(toDecimal(toBigInt(2000 * 4, 6), 6));
