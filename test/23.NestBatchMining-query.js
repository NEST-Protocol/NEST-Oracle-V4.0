...
describe('NestOpenMining', function() {
    it('First', async function() {
        ...
        await usdc.transfer(owner.address, 10000000000000000000000000n);
        await cofi.transfer(owner.address, 10000000000000000000000000n);
        ...
        await nestBatchMining.addPair(0, usdc.address);
        await nestBatchMining.addPair(0, cofi.address);
        console.log(await getStatus());
        ...
        if (true) {
            console.log('1. post');
            let receipt = await nestBatchMining.post(0, 1, [60000000000n, 700000000000n, 8000000000000000000000n], {
            ...
            expect(status.owner.hbtc).to.eq(toDecimal(10000000000000000000000000n - 1000000000000000000n * N));
            expect(status.owner.nest).to.eq(toDecimal(4000000000000000000000000000n - OPEN_FEE - 100000000000000000000000n * N));
            expect(status.mining.usdt).to.eq(toDecimal(60000000000n, 6));
            expect(status.mining.hbtc).to.eq(toDecimal(1000000000000000000n * N));
            expect(status.mining.nest).to.eq(toDecimal(5000000000000000000000000000n + OPEN_FEE + 100000000000000000000000n * N));
            ...
            expect(status.mining.hbtc).to.eq(toDecimal(1000000000000000000n + 1000000000000000000n * (1n + N) - 1000000000000000000n));
            expect(status.mining.nest).to.eq(toDecimal(5000000000000000000000000000n + OPEN_FEE + 100000000000000000000000n * (2n + N)));
            expect(status.mining.eth).to.eq(toDecimal(toBigInt(POSTFEE) + 1000000000n * GASLIMIT));
            ...
            if (false) {
                ...
                expect(status.owner.hbtc).to.eq(toDecimal(10000000000000000000000000n - 1000000000000000000n * N - 1000000000000000000n * 2n));
                expect(status.owner.nest).to.eq(toDecimal(4000000000000000000000000000n - OPEN_FEE - 100000000000000000000000n * N - toBigInt(100000 * 4)));
                expect(status.addr1.usdt).to.eq(toDecimal(10000000000000n - 70000000000n * 2n - 60000000000n, 6));
                expect(status.addr1.hbtc).to.eq(toDecimal(10000000000000000000000000n - 1000000000000000000n * 2n + 1000000000000000000n));
                expect(status.addr1.nest).to.eq(toDecimal(1000000000000000000000000000n - 100000000000000000000000n * 2n));
                expect(status.mining.usdt).to.eq(toDecimal(60000000000n + 70000000000n * 2n + 60000000000n + 65000000000n * 4n + 70000000000n * 2n, 6));
                expect(status.mining.hbtc).to.eq(toDecimal(1000000000000000000n + 1000000000000000000n * (2n + N - 1n) - 1000000000000000000n + 1000000000000000000n * 2n));
                expect(status.mining.nest).to.eq(toDecimal(5000000000000000000000000000n + OPEN_FEE + 100000000000000000000000n * N + 100000000000000000000000n * 2n + toBigInt(100000 * 4)));
                expect(status.mining.eth).to.eq(toDecimal(toBigInt(POSTFEE) + 1000000000n * GASLIMIT));
                ...
                await nestBatchMining.close(0, [[0, 2], [0], [0]]);
                await nestBatchMining.close(0, [[1], [], []]);
                status = await showStatus();
                ...
                expect(toDecimal(await nestBatchMining.balanceOf(hbtc.address, owner.address))).eq(toDecimal(toBigInt(4n + N - 1n)));
                expect(toDecimal(await nestBatchMining.balanceOf(usdt.address, owner.address), 6)).eq(toDecimal(toBigInt(65000*4+60000*2, 6), 6));
                expect(toDecimal(await nestBatchMining.balanceOf(nest.address, owner.address))).eq(toDecimal(toBigInt(100000n * N + 400000n + 10n)));
